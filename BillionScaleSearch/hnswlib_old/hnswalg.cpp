    ...
    HierarchicalNSW::HierarchicalNSW(size_t d, size_t maxelements, size_t M, size_t maxM, size_t efConstruction)
{
    ...
    HierarchicalNSW::HierarchicalNSW(const std::string & infoLocation, const std::string & edgeLocation, std::vector<float>& Data){
        LoadInfo(infoLocation);
        for (size_t i = 0; i < maxelements_; i++){
            memcpy(getDataByInternalId(i), Data.data() + i * d_, data_size_);
        }
        LoadEdges(edgeLocation);
    }
...
void HierarchicalNSW::mutuallyConnectNewElement(idx_t cur_c,
                               std::priority_queue<std::pair<float, idx_t>> topResults)
{
...
{
    size_t result_num = efSearch > k ? efSearch : k;
    auto topResults = searchBaseLayer(query, result_num);
...
void HierarchicalNSW::SaveInfo(const std::string &location, bool visualize)
{
    if (visualize)
    ...
    writeBinaryPOD(output, efSearch);
}


void HierarchicalNSW::SaveEdges(const std::string &location, bool visualize)
{
    if (visualize)
...
void HierarchicalNSW::LoadData(const std::string &location)
{
    ...
    for (size_t i = 0; i < maxelements_; i++) {
        ...
        if (dim != d_) {
            std::cout << "Wrong data dim: Expected: " << d_ << " Actual: " << dim << std::endl;
            exit(1);
        }
